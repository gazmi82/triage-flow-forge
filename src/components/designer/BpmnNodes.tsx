...
export function AndGatewayNode({ data, selected }: NodeProps) {
  return (
    <div className="relative flex h-12 w-12 items-center justify-center" style={{ transform: "rotate(45deg)" }}>
      ...
      <Handle type="source" position={Position.Right} style={{ transform: "rotate(-45deg) translate(8px, 0)" }} className="!bg-node-gateway-and !border-node-gateway-and" />
      <Handle type="source" id="bottom" position={Position.Bottom} style={{ transform: "rotate(-45deg) translate(0, 8px)" }} className="!bg-node-gateway-and !border-node-gateway-and" />
    </div>
  );
}
